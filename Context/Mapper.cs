...
namespace epiwebsurvey_api.Context
{
    public static class Mapper
    {
        ...
        public static SurveyInfoBO Map(SurveyMetaData entity)
        {
            ...
            try
            {
                result.OrganizationKey = Guid.Parse(Cryptography.Decrypt(entity.Organization.OrganizationKey.ToString()));
            }
            catch (Exception ex) { }
        ...
        internal static SurveyResponse ToEF(SurveyResponseBO pBO, int OrgId = -1)
        {
            ...
            if (!string.IsNullOrEmpty(pBO.Json))
            {
                SurveyResponse.ResponseJson = pBO.Json;               
            }            
